angular.module('IOne-Production').config(['$routeProvider', function ($routeProvider) {
    $routeProvider.when('/refund2s', {
        controller: 'Refund2sController',
        templateUrl: 'app/src/app/order/refund2/refund2.html'
    })
}]);

angular.module('IOne-Production').controller('Refund2sController', function ($scope, OrderMaster, Receipt2s, Constant, $mdDialog, $q) {
    $scope.pageOption = {
        sizePerPage: 10,
        currentPage: 0,
        totalPage: 100,
    ...
    $scope.refreshList = function () {
        OrderMaster.getAll($scope.pageOption.sizePerPage, $scope.pageOption.currentPage, $scope.listFilterOption.confirm, '', '', $scope.listFilterOption.no, '', '', '', '', '', $scope.listFilterOption.channelUuid, $scope.listFilterOption.orderAmount, $scope.listFilterOption.paidAmount, $scope.listFilterOption.unpaidAmount, null, '2').success(function (data) {
    ...
    $scope.detailConfirmClickAction = function (event, detail) {
        $scope.stopEventPropagation(event);
        if (detail.status == 2) {
            $scope.showConfirm('确认取消审核退款单身吗？', '', function () {
                var UpdateInput = {
                    uuid: detail.uuid,
                    status: '1'
                };
                Receipt2s.modify(detail.orderMaster.uuid, UpdateInput).success(function (data) {
                    detail.status = '1';
                    $scope.showInfo('取消审核成功！');
                }).error(function (response) {
                    $scope.showError(response.message);
            ...
            $scope.showConfirm('确认审核退款单身吗？', '', function () {
                var UpdateInput = {
                    uuid: detail.uuid,
                    status: '2'
                };
    ...
    $scope.detailTransferClickAction = function (event, detail) {
        ...
        if (detail.transferFlag == 2) {
            $scope.showConfirm('确认重新抛转退款单身吗？', '', function () {
                var UpdateInput = {
                    uuid: detail.uuid,
            ...
            $scope.showConfirm('确认抛转退款单身吗？', '', function () {
                var UpdateInput = {
                    uuid: detail.uuid,
                    transferFlag: '2',
                    receiptOrderConversion: 'true'
