 ...
 */
angular.module('IOne-Production').service('UserService', function ($http, Constant) {
    this.getAll = function (sizePerPage, page, filter) {
        ...
        var url = 'users?size=' + sizePerPage
            + '&page=' + page;
    ...
    this.modify = function (uuid, dutyUpdateInput) {
        return $http.patch(Constant.BACKEND_BASE + '/users/' + uuid, dutyUpdateInput);
    };

    this.add = function (dutyInput) {
        return $http.post(Constant.BACKEND_BASE + '/users/', dutyInput);
    };
