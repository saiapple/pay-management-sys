angular.module('IOne-Production').config(['$routeProvider', function ($routeProvider) {
    $routeProvider.when('/fam/bank', {
        controller: 'FAMBankConfigController',
        templateUrl: 'app/src/app/fam/bank/bank.html'
    })
}]);

angular.module('IOne-Production').controller('FAMBankConfigController', function ($scope, FAMBankService, Constant, $mdDialog, $q) {
    $scope.pageOption = {
        sizePerPage: 10,
        currentPage: 0,
        totalPage: 100,
    ...
    $scope.menuDisplayOption = {
        'switchConfirm': {display: true, name: '审核/取消审核', uuid: '146c53c8-c1aa-4797-92e1-1f2d72669c8e'},
        'switchStatus': {display: true, name: '启用/禁用', uuid: 'eb389d3f-3e24-463f-8caf-0aca81a4613c'},

        'batchConfirm': {display: true, name: '批量审核', uuid: 'c74babb4-effd-4ab1-bde0-b288bcdefa33'},
        'batchRevertConfirm': {display: true, name: '批量取审', uuid: '2b5254f8-6d89-4645-b108-4dbc617a204c'},
        'batchStatus': {display: true, name: '批量启用', uuid: '32a38bbe-1a60-4232-99e0-40a872434989'},
        'batchRevertStatus': {display: true, name: '批量禁用', uuid: 'a909235f-729d-456f-933a-1e5553c893a4'},
        'batchDelete': {display: true, name: '批量删除', uuid: 'ece38745-577a-4b0f-b0dd-b35c8544bbfe'},

        'detailConfirm': {display: true, name: '审核', uuid: '11ab07d6-81a1-4591-b6b1-3a1c3b678367'},
        'detailRevertConfirm': {display: true, name: '取审', uuid: '5dcae7eb-dd36-4eb4-b002-af65610b0ff1'},
        'detailStatus': {display: true, name: '启用', uuid: 'e22ca984-0517-44be-8373-98650dc79b19'},
        'detailRevertStatus': {display: true, name: '禁用', uuid: 'dacd23c4-0207-4b3b-83da-242712b6fd28'},
        'detailDelete': {display: true, name: '删除', uuid: 'f8c196fb-98bc-4d1f-a534-ae0b53e10c6f'}
    ...
    $scope.refreshList = function () {
        FAMBankService.getAll($scope.pageOption.sizePerPage, $scope.pageOption.currentPage,
    ...
    $scope.saveItemAction = function () {
        if ($scope.status == 'add') {
            if ($scope.domain == 'FAM_BASE_BANK') {
                FAMBankService.add($scope.source).success(function (data) {
            ...
            if ($scope.domain == 'FAM_BASE_BANK') {
                FAMBankService.modify($scope.source.uuid, $scope.source).success(function (data) {
    ...
    $scope.confirmClickAction = function (event, item) {
        ...
        if (item.confirm == Constant.CONFIRM[2].value) {
            $scope.showConfirm('确认取消审核吗？', '', function () {
                ...
                FAMBankService.modify(item.uuid, UpdateInput).success(function () {
    ...
    $scope.confirmSwitchAction = function (event, item) {
        ...
        if (item.confirm == Constant.CONFIRM[2].value) {
            $scope.showConfirm('确认取消审核吗？', '', function () {
                ...
                FAMBankService.modify(UpdateInput.uuid, UpdateInput).success(function () {
                    $scope.disableBatchMenuButtons();
                    $scope.showInfo('修改数据成功。');
                });
            }, function () {
    ...
    $scope.deleteClickAction = function (event, item) {
        ...
        $scope.showConfirm('确认删除吗？', '删除后不可恢复。', function () {
            FAMBankService.delete(item.uuid).success(function () {
                $scope.selectedItem = null;
                $scope.refreshList();
    ...
    $scope.deleteAllClickAction = function (event) {
        ...
        angular.forEach($scope.itemList, function (item) {
            if (item.selected === true) {
                    var response = FAMBankService.delete(item.uuid).success(function () {
                    });
        ...
        angular.forEach($scope.itemList, function (item) {
            if (item.selected === true) {
                if (item.confirm != Constant.CONFIRM[2].value) {
                    ...
                    var response = FAMBankService.modify(item.uuid, UpdateInput).success(function () {
                    });
                    promises.push(response);
                    count++;
                } else {
